...
function create_output_initialization(y::Vector{Fl}, X::Union{Matrix{Fl}, Missing}, gas_model::GASModel) where {Fl}
    ...
    has_HS_seasonal = zeros(Bool, num_params)
    ...
    for i in idx_time_varying_params
        ...
        has_HS_seasonal[i] = has_HS_seasonality(seasonality, i)
        if has_seasonal[i] && !has_HS_seasonal[i]
            seasonal_period[i] = get_num_harmonic_and_seasonal_period(seasonality_dict)[2][i]
        elseif has_HS_seasonal[i]
            seasonal_period[i] = get_HS_seasonal_period(seasonality_dict)[i]
        end

        X_aux =  i == 1 && !ismissing(X) ? X : missing

        if i != 2
            initial_values[i] = get_initial_values(initial_params[i], X_aux, has_level[i], has_level_ar1[i], has_slope[i], has_seasonal[i], seasonal_period[i], true, order[i], max_order)
        end
    end
